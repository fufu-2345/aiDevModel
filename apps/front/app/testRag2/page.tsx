...
interface GenPromptResponse {
  sd_prompt: string;
  image_url?: string; // รับ URL รูปภาพ
  message?: string;
}
...
export default function RAGTester() {
  ...
  const handleTriggerRAG = async () => {
    ...
    try {
      ...
      setRagResult({ type: "success", data: res.data });
    } catch (error: any) {
      console.error(error);
      setRagResult({
        type: "error",
  ...
  const handleGenPrompt = async () => {
    ...
    try {
      // ส่ง parameter save_image=true
      const res = await axios.post<GenPromptResponse>(
        `${API_BASE_URL}/gen-image-prompt?movie_id=${movieId}&scene_query=${sceneQuery}&save_image=true`
      );
      setGeneratedPrompt(res.data.sd_prompt);

      if (res.data.image_url) {
        // แปะ URL ของ Backend เพื่อแสดงรูป
        setGeneratedImage(`${API_BASE_URL}${res.data.image_url}`);
      }
    } catch (error) {
      console.error(error);
      alert("เชื่อมต่อ Backend ไม่ได้ หรือ SD API ไม่ได้เปิดไว้");
      setGeneratedPrompt("Error Generating Prompt/Image");
    } finally {
      ...
      <div className="max-w-3xl mx-auto space-y-10">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500">
            ทดสอบระบบนิยาย AI + Stable Diffusion (Local)
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="bg-indigo-600 px-6 py-4 flex justify-between items-center">
            <h2 className="text-white font-semibold flex items-center gap-2">
              <PlayIcon size={20} /> 1. สั่งประมวลผล RAG
            </h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="bg-pink-600 px-6 py-4 flex justify-between items-center">
            <h2 className="text-white font-semibold flex items-center gap-2">
              <ImageIcon size={20} /> 2. สร้าง Prompt & Gen ภาพจริง
            </h2>
          ...
          <div className="p-6 space-y-4">
            ...
            >
              {promptLoading ? (
                <>
                  <SpinnerIcon className="animate-spin" size={18} />{" "}
                  กำลังสร้างภาพ (อาจใช้เวลา 10-20 วิ)...
                </>
              ) : (
                <>
                  <ImageIcon size={18} /> Generate Prompt & Image
                </>
              )}
            </button>

            {/* แสดงรูปภาพที่ Save มา */}
            {generatedImage && (
              <div className="mt-4 p-4 bg-slate-50 border border-slate-200 rounded-lg text-center">
                ...
                />
                <p className="text-xs text-slate-400 mt-2">{generatedImage}</p>
              </div>
            )}
            ...
            {generatedPrompt && (
              <div className="mt-6 space-y-2">
                ...
                <div className="p-4 bg-slate-900 text-green-400 font-mono text-sm rounded-lg border border-slate-700 break-words leading-relaxed shadow-inner">
                  {generatedPrompt}
                </div>
              </div>
            )}
